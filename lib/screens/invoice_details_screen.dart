...
class InvoiceDetailsScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body:LayoutBuilder(builder:(_,box)=>ListView(padding:EdgeInsets.symmetric(horizontal:box.maxWidth>760?56:16,vertical:20),children:[
        ...
        _card('Company information',[_info(invoice.company.companyName,invoice.company.address,invoice.company.email,invoice.company.phone)]),
  ...
  Widget _info(String title,String address,String email,String phone)=>Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(title,style:const TextStyle(fontWeight:FontWeight.w600)),if(address.isNotEmpty)Text(address),if(email.isNotEmpty)Text(email),if(phone.isNotEmpty)Text(phone)]);
